...
```bash
cd frontend && npx convex env set BACKEND_SECRET_KEY your_backend_secret_key_here
```
...
- **`frontend/`**: Reorganized React frontend and Convex database files.
  ...
  - **`index.html`**: HTML entry point for the Vite web application.
  - **`vite.config.ts` & `tsconfig.json`**: Bundling and strict TypeScript configurations.
  - **`package.json` & `package-lock.json`**: Node package descriptors and lockfile.
  - **`.env.local`**: Local environment configurations generated by the Convex compiler.
- **`architecture.md`**: Inside-out detailed engineering reference guide.
- **`design.md`**: CSS styling tokens and theme reference guide.
