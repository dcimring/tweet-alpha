...
- **Build Environment Configuration (`nixpacks.toml`)**: Instructs Nixpacks during the setup phase to install `curl`, download and install the `xurl` binary from its installer script, and permanently append the `xurl` binary directory to the system `PATH` env var (`/home/nixpacks/.local/bin:$PATH`).
